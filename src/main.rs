...
async fn main() -> Result<()> {
    ...
    if args.len() > 1 {
        match args[1].as_str() {
            ...
            "usage" => { println!("{}", mcp::get_detailed_guide()); return Ok(()); }
            "update" => {
            ...
            "mcp-doc" => {
                ...
                let content = format!(r#"# 🤖 DeckDriod MCP Guide for AI Assistants

This document explains how to connect an AI assistant to DeckDriod using the Model Context Protocol (MCP).

## 1. Start the MCP Server
Inside DeckDriod, press **Shift + M**. 
The header will show `[MCP:{}]` when active.
...
- **Tools**: `get_recent_logs`, `get_errors`, `get_latest_crash`, `get_usage_guide`.
- **Live Guide**: Visit `http://localhost:{}/usage` in your browser when the server is ON.

---
Generated by DeckDriod v{}
"#, port, port, port, port, env!("CARGO_PKG_VERSION"));
